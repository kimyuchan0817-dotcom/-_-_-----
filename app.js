...
// ─────────────────────────────────────────────
//  내장 한국 공휴일 데이터 (2024~2027)
//  API 없이도 항상 정확하게 동작합니다.
// ─────────────────────────────────────────────
...
function init() {
    ...
    // 내장 공휴일 데이터 로드 (API 없이 항상 동작)
    holidayMap = { ...BUILTIN_HOLIDAYS };
    // 추가로 캐시된 API 공휴일이 있으면 병합
    const cachedHolidays = localStorage.getItem('campusflow_holidays');
    if (cachedHolidays) {
        try {
            const cachedMap = JSON.parse(cachedHolidays);
            holidayMap = { ...holidayMap, ...cachedMap };
        } catch(e) {}
    }
...
function renderCalendar(filteredTasks) {
    ...
    // API 키가 있을 때만 추가 공휴일 fetch 시도 (내장 데이터는 이미 표시됨)
    if (holidayApiKey) {
        fetchHolidaysForMonth(holidayApiKey, calendarYear, calendarMonth + 1)
            .then(() => {
                const newCount = Object.keys(holidayMap).length;
                if (newCount > Object.keys(BUILTIN_HOLIDAYS).length) renderCalendar(filteredTasks);
            })
            .catch(() => {}); // silent — built-in data still shows
    }
}
...
async function loadHolidaysForCurrentView() {
    ...
    try {
        ...
        if (statusEl) {
            ...
            statusEl.textContent = `✓ 공휴일 ${count}건 로드됨`;
        }
        render();
    } catch (err) {
        if (statusEl) {
...
function bindHolidayApiUI() {
    // Always show built-in holiday count in status
    const statusEl = document.getElementById('holidayApiStatus');
    if (statusEl) {
        statusEl.className = 'calendar-api-status ok';
        statusEl.textContent = `✓ 내장 공휴일 ${Object.keys(BUILTIN_HOLIDAYS).length}건 (2024~2027)`;
    }

    // API key input and button are optional (inside <details>) — attach lazily
    const keyInput = document.getElementById('holidayApiKey');
    if (keyInput) {
        if (holidayApiKey) keyInput.value = holidayApiKey;
